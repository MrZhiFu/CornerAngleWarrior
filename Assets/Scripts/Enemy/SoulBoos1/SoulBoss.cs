...
public class SoulBoss : MonoBehaviour {
    ...
    private CharacterController cc;//Boss的角色控制器
    private Animator animator;//动画控制器

    public float attackDistance = 2.0f;//Boss的攻击距离
    public float moveSpeed = 2.0f;//Boos的移动速度
	...
	void Update () {
        ...
        if (distance<= attackDistance)//在攻击距离之内可以攻击主角
        {
            ...
            {
                int temp = Random.Range(0, 2);//随机发动不同攻击
                if (temp == 0)
                {
